...
## different functions highlighting how our data is going to be used or analyzed  
...
##the function below helps us create a 2 by 2 table with a chart in each cell
par(mfrow = c(2,2)) # four plots on one window/page

#the getsymbol below is a function that helps us load an manage data from multiple sources
...
##the date format is set based on the earliest (min.) and the latest date(max)
##the next function dimnames helps hels set the name for rows or columns 
##the ts function is the time and series function allowing us to convert numeric vectors into an R time series object
dimnames(ER)[2] <- "ER"
